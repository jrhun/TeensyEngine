...
public:
	PatternFlowers() : _Pattern("Flowers") {}
	...
	uint8_t drawFrame() {
		...
		uint8_t hue = FxFade;
		drawFlower(10, 0, currentFrame, hue);// hues[currentHue]);

		//change hue at end of frame counter
		frameCounter = (frameCounter + 1) % frameCounterMax;
		if (frameCounter == 0) {
			currentHue = (currentHue + 1) % maxHues;
		}

		return returnVal;
	}

	void drawFlower(uint8_t x, uint8_t y, uint8_t frame, uint8_t hue) {
		...
		CRGB flower0 = CRGB(240, 161, 41);
		CRGB flower1 = CRGB(245, 198, 107);
		CRGB flower2 = CRGB(244, 224, 162);
		if (frame == 2) {
			gfx.putPixel(flowerTopOffsetX + x + 4, flowerTopOffsetY + y + 4, flower0);
		}
		else if (frame == 3) {
			gfx.putPixel(flowerTopOffsetX + x + 4, flowerTopOffsetY + y + 4, flower0);
			gfx.putPixel(flowerTopOffsetX + x + 3, flowerTopOffsetY + y + 4, flower1);
			gfx.putPixel(flowerTopOffsetX + x + 4, flowerTopOffsetY + y + 3, flower1);
		}
		else if (frame == 4) {
			gfx.putPixel(flowerTopOffsetX + x + 4, flowerTopOffsetY + y + 4, flower0);
			gfx.putPixel(flowerTopOffsetX + x + 3, flowerTopOffsetY + y + 4, flower1);
			gfx.putPixel(flowerTopOffsetX + x + 4, flowerTopOffsetY + y + 3, flower1);
			gfx.putPixel(flowerTopOffsetX + x + 3, flowerTopOffsetY + y + 3, flower2);
		}
	}

	uint8_t currentFrame = 0;
	const uint8_t maxFrames = 5;
	uint8_t frameCounter = 0;
	uint8_t frameCounterMax = 240;
	...
	const uint8_t hues[maxHues] = {
		//0, 27, 41, 138, 156, 191, 213
		0, 10, 20, 30, 120, 191, 220
	};
	uint8_t currentHue = 0;


	const uint16_t* flowerBase[5] = {
		//R5G6B5
	...
	const uint8_t* flowerTop[5] = {
		//Grey8
		flower_top1_8_11_8bit,
		flower_top2_8_11_8bit,
		flower_top3_8_11_8bit,
		flower_top4_8_11_8bit,
		flower_top5_8_11_8bit
	};
};
